...
rsync -avz electron/package.json -avz electron/hope-mac-5/package.json
cd electron/hope-mac-5
sed -i .bak 's/APP_NAME/hope-mac-5/g' package.json # replace app name in package

# install dependencies
sudo npm install --unsafe-perm=true --allow-root
sudo npm rebuild --runtime=electron --target=3.1.8 --disturl=https://atom.io/download/atom-shell --abi=48 # for robotjs, we need to indicate the electron version
cd ../..
